...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	opt_delay(2);	  
 	...
 	if(lcddev.id==0X9341||lcddev.id==0X9486||lcddev.id==0X5310||lcddev.id==0X5510)		//9341/NT35310/NT35510要分2次读出
 	{
		opt_delay(2);	  
		b=LCD_RD_DATA(); 
	...
	else if(lcddev.id==0X9341||lcddev.id==0X9486||lcddev.id==0X5310||lcddev.id==0X5510)return (((r>>11)<<11)|((g>>10)<<5)|(b>>11));//ILI9341/NT35310/NT35510需要公式转换一下
...
//	FSMC_Bank1->BTCR[6]|=1<<0;		//使能BANK1，区域1	    
	...
	FSMC_Bank1E->BWTR[0]|=9<<0;		//地址建立时间(ADDSET)为9个HCLK=54ns
 	//9个HCLK（HCLK=168M）,某些液晶驱动IC的写信号脉宽，最少也得50ns。  	 
	FSMC_Bank1E->BWTR[0]|=9<<8; 	//数据保存时间(DATAST)为6ns*9个HCLK=54ns
	//使能BANK1,区域1
...
//  	lcddev.id = LCD_ReadReg(0x0000);   
	...
	if(lcddev.id==0X9341||lcddev.id==0X9486||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963)//如果是这几个IC,则设置WR时序为最快
	{
		//重新配置写时序控制寄存器的时序   	 							    
	...
	}else if(lcddev.id==0X6804||lcddev.id==0XC505)	//6804/C505速度上不去,得降低
	{
		...
		FSMC_Bank1E->BWTR[0]&=~(0XF<<0);//地址建立时间(ADDSET)清零 	 
		FSMC_Bank1E->BWTR[0]&=~(0XF<<8);//数据保存时间清零
		FSMC_Bank1E->BWTR[0]|=10<<0;	//地址建立时间(ADDSET)为10个HCLK =60ns  	 
		FSMC_Bank1E->BWTR[0]|=12<<8; 	//数据保存时间(DATAST)为6ns*13个HCLK=78ns
	}
 	//printf(" LCD ID:%x\r\n",lcddev.id); //打印LCD ID   
